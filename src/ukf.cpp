 ...
 */
UKF::UKF() {
  is_initialized_ = false; // default this value to false until explicitly true.
  time_us_ = 0.0; // default this value to 0 until intialized
  ...
  n_x_ = 5; // length of the state vector
  n_aug_ = 7; // length of the augmented state vector
  lambda_ = 3 - n_aug_; // sigma point spreading function
  lambda_aug_ = sqrt(lambda_ + n_aug_);
  sigma_points_ = 2 * n_aug_ + 1;

  x_ = VectorXd(n_x_);
  x_.fill(0.0); // initialize state vector [px, py, v, yaw, yaw_dot]

  P_ = MatrixXd(n_x_, n_x_);
  P_.fill(0.0);
  for (int i=0; i<n_x_; i++) {P_(i, i) = 0.5;} // initialize covariance matrix
  ...
  Xsig_pred_ = MatrixXd(n_x_, sigma_points_); // initial X sigma prediction matrix
  Xsig_pred_.fill(0.0); // initialize X sigma prediction matrix to 0.0

  std_a_ = 1.25; // Process noise standard deviation longitudinal acceleration in m/s^2
  std_yawdd_ = 0.25; // Process noise standard deviation yaw acceleration in rad/s^2
